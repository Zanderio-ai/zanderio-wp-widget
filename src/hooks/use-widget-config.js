...
export function useWidgetConfig(settings, remoteConfig) {
  ...
  const widgetConfig = useMemo(() => {
    ...
    if (source) {
      return {
        name: source.name || settings.widgetName || "Zanderio",
        icon:
          source.logo_url || source.icon || settings.widgetIcon || zanderioIcon,
        color:
          source.color ||
        ...
        welcomeMessage:
          ...
          "Hi there!",
        desktopPosition:
          source.desktop_position || source.desktopPosition || "bottom-right",
        mobilePosition:
          source.mobile_position || source.mobilePosition || "bottom-center",
        labelText: source.label_text || source.labelText,
        buttonType: source.button_type || source.buttonType,
      };
    }
